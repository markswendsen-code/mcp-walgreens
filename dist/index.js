...
// ─── Server setup ─────────────────────────────────────────────────────────────
const server = new index_js_1.Server({ name: "walgreens", version: "0.1.0" }, { capabilities: { tools: {} } });
// ─── Tool definitions ─────────────────────────────────────────────────────────
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "login",
            description: "Authenticate with Walgreens account using email and password via browser automation",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "checkout",
            description: "Proceed to checkout and return order summary. Does NOT auto-confirm — returns summary for review.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "search":
                return await handleSearch(a.query, a.category, a.min_price, a.max_price, Math.min(a.limit ?? 10, 24));
            case "get_product":
                return await handleGetProduct(a.url, a.product_id);
            case "check_prescription_status":
                return await handleCheckPrescriptionStatus(a.rx_number);
            case "refill_prescription":
                return await handleRefillPrescription(a.rx_number);
            case "add_to_cart":
                return await handleAddToCart(a.url, a.product_id, a.quantity ?? 1);
            case "view_cart":
                return await handleViewCart();
            case "remove_from_cart":
                return await handleRemoveFromCart(a.item_index, a.product_name);
            case "set_store":
                return await handleSetStore(a.zip_code, a.store_id);
            case "check_store_availability":
                return await handleCheckStoreAvailability(a.url, a.product_id, a.zip_code);
            case "checkout":
...
async function handleStatus() {
    ...
    const store = (0, session_js_1.loadStore)();
    if (!loggedIn) {
        return ok("Not logged in. Use the `login` tool to authenticate with your Walgreens account.\n" +
...
async function handleLogin(email, password, headless) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        const accountEl = await page.$('[data-testid="account-name"], .account-name, [aria-label*="account"], [class*="myAccount"], [class*="my-account"]');
        if (accountEl) {
        ...
        // Fill username/email
        const emailInput = await page.waitForSelector('input[id="user_name"], input[name="user_name"], input[id="userName"], input[name="userName"], input[type="email"], input[id="email"]', { timeout: 15000 });
        await emailInput.click();
        await emailInput.fill(email);
        await page.waitForTimeout(500);
        // Fill password
        const passwordInput = await page.waitForSelector('input[id="user_password"], input[name="user_password"], input[type="password"], input[id="password"]', { timeout: 10000 });
        await passwordInput.click();
        await passwordInput.fill(password);
        // Submit
        const submitBtn = await page.waitForSelector('button[type="submit"], input[type="submit"], button[id*="login"], button[class*="login"], #submit_btn', { timeout: 5000 });
        await submitBtn.click();
        // Wait for navigation
        await page.waitForTimeout(3500);
        // Check for error messages
        const errorEl = await page.$('[class*="error"], [id*="error"], .errorMsg, .error-message, [data-testid*="error"]');
        if (errorEl) {
        ...
        let name;
        try {
            const nameEl = await page.$('[class*="account-name"], [data-testid="account-name"], .welcome-message, [class*="greeting"], [class*="myAccount"]');
            if (nameEl)
...
async function handleSearch(query, category, minPrice, maxPrice, limit = 10) {
    return (0, browser_js_1.withPage)(async (page) => {
        const params = new URLSearchParams({ Ntt: query });
        if (category)
            params.set("N", category);
        const searchUrl = `https://www.walgreens.com/search/results.jsp?${params.toString()}`;
        await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
        await page.waitForTimeout(2500);
        // Wait for product results
        try {
            await page.waitForSelector('[class*="product-list"], [class*="productList"], [class*="search-results"], .product-card, [class*="productCard"], [data-testid*="product"]', { timeout: 15000 });
        }
        catch {
        ...
        const products = await page.evaluate(({ minPrice, maxPrice, limit }) => {
            ...
            for (const card of cards) {
                if (results.length >= limit)
                    break;
                const titleEl = card.querySelector('[class*="product-name"], [class*="productName"], [class*="product__name"], h3, h4, p[class*="title"]');
                ...
                if (maxPrice && !isNaN(priceNum) && priceNum > maxPrice)
                    continue;
                const linkEl = card.querySelector('a[href*="/store/product"], a[href*="/store/c/"]');
                ...
                const ratingEl = card.querySelector('[class*="rating"], [aria-label*="out of"], [class*="stars"]');
...
async function handleGetProduct(url, productId) {
    if (!url && !productId) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        await page.waitForTimeout(2000);
        try {
            await page.waitForSelector('h1, [class*="product-title"], [class*="productTitle"], [class*="product__name"], [class*="pdp"]', { timeout: 15000 });
        }
        catch {
            return err("Product page failed to load");
        }
        const product = await page.evaluate(() => {
            const title = document.querySelector('[class*="product-title"], [class*="productTitle"], [class*="product__name"], h1')
                ?.textContent?.trim() ?? "";
            const priceEl = document.querySelector('[class*="price"], [class*="Price"], [data-testid*="price"], .product__price');
            const price = priceEl?.textContent?.trim() ?? "";
            const brand = document.querySelector('[class*="brand"], [class*="Brand"], [class*="manufacturer"]')?.textContent?.trim() ?? "";
            const description = document.querySelector('[class*="description"], [id*="description"], [data-testid="description"], [class*="product-detail"], [class*="productDetail"]')?.textContent?.trim()?.slice(0, 600) ?? "";
            const ratingEl = document.querySelector('[class*="rating"], [class*="Rating"], [class*="stars"]');
            const rating = ratingEl?.getAttribute("aria-label") ?? ratingEl?.textContent?.trim() ?? "";
            const reviewsEl = document.querySelector('[class*="review-count"], [class*="reviewCount"], [class*="reviews-count"]');
            const reviews = reviewsEl?.textContent?.trim() ?? "";
            const availabilityEl = document.querySelector('[class*="availability"], [class*="Availability"], [class*="stock"], [data-testid="availability"], [class*="fulfillment"]');
            const availability = availabilityEl?.textContent?.trim()?.slice(0, 200) ?? "";
            const images = Array.from(document.querySelectorAll('[class*="product-image"] img, [class*="productImage"] img, [class*="gallery"] img, [class*="carousel"] img'))
                .map((img) => img.src)
...
async function handleCheckPrescriptionStatus(rxNumber) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        if (rxNumber) {
            try {
                const searchInput = await page.$('input[placeholder*="prescription" i], input[name*="rx" i], input[id*="rx" i], input[placeholder*="rx number" i]');
                if (searchInput) {
        ...
        const prescriptions = await page.evaluate((filterRx) => {
            const rxSelectors = [
            ...
            if (rxEls.length === 0) {
                rxEls = Array.from(document.querySelectorAll('tr[class*="rx"], .rx-row, [role="row"]')).filter((el) => el.textContent?.match(/\d{7,}/));
            }
            return rxEls.map((el) => {
                const rxNum = el.querySelector('[class*="rx-number"], [class*="rxNumber"], [class*="prescription-number"]')?.textContent?.trim() ??
                    el.textContent?.match(/Rx[:\s#]*(\d+)/i)?.[1] ?? "";
                const drug = el.querySelector('[class*="drug-name"], [class*="drugName"], [class*="medication"], [class*="medName"], h3, h4')?.textContent?.trim() ??
                    "";
                const status = el.querySelector('[class*="status"], [class*="Status"]')?.textContent?.trim() ?? "";
                const refillsLeft = el.querySelector('[class*="refills"], [class*="Refills"], [class*="refills-remaining"]')?.textContent?.trim() ?? "";
                const daysSupply = el.querySelector('[class*="days-supply"], [class*="daysSupply"]')?.textContent?.trim() ?? "";
                const lastFilled = el.querySelector('[class*="last-fill"], [class*="lastFill"], [class*="fill-date"], [class*="fillDate"]')?.textContent?.trim() ??
                    "";
        ...
        if (prescriptions.length === 0) {
            ...
            return ok(`No prescriptions found${rxNumber ? ` for Rx #${rxNumber}` : ""}.\n\n` +
        ...
        prescriptions.forEach((rx, i) => {
            lines.push(`${i + 1}. ${rx.drug || "Unknown medication"}\n` +
...
async function handleRefillPrescription(rxNumber) {
    if (!rxNumber) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        for (const rxEl of rxEls) {
            ...
            if (text?.includes(rxNumber)) {
                const refillBtn = await rxEl.$('button[class*="refill" i], button[aria-label*="refill" i], a[class*="refill" i], [data-testid*="refill"]');
                if (refillBtn) {
        ...
        if (!refillClicked) {
            ...
            await page.waitForTimeout(2000);
            const refillBtn = await page.$('button[class*="refill" i], button[type="submit"], [data-testid="refill-submit"]');
            if (refillBtn) {
                await refillBtn.click();
                await page.waitForTimeout(2000);
                refillClicked = true;
        ...
        if (!refillClicked) {
            return err(`Could not find refill option for Rx #${rxNumber}. ` +
                "Please verify the prescription number using `check_prescription_status`.");
        }
        // Confirm modal if present
        try {
            const confirmBtn = await page.waitForSelector('button[class*="confirm" i], button[data-testid*="confirm"], button[aria-label*="confirm" i]', { timeout: 3000 });
            if (confirmBtn) {
        ...
        const successEl = await page.$('[class*="success"], [class*="confirmation"], [data-testid*="success"], .alert-success');
        const successText = successEl ? await successEl.textContent() : null;
        if (successText) {
            return ok(`Refill requested for Rx #${rxNumber}.\n\n${successText.trim()}`);
        }
        return ok(`Refill request submitted for Rx #${rxNumber}.\n` +
            `Please check 'check_prescription_status' to confirm the refill is being processed.`);
    });
}
async function handleAddToCart(url, productId, quantity = 1) {
    if (!url && !productId) {
        return err("Provide either url or product_id");
    }
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        if (quantity > 1) {
            try {
                const qtyInput = await page.$('input[aria-label*="quantity" i], input[name*="quantity"], select[name*="quantity"], [data-testid*="quantity"] input');
                if (qtyInput) {
        ...
        const addBtn = await page.waitForSelector('button[class*="add-to-cart" i], button[aria-label*="add to cart" i], [data-testid*="add-to-cart"], button[id*="addToCart"], button[class*="addToCart"]', { timeout: 10000 });
        ...
        const confirmation = await page.$('[class*="cart-count"], [aria-label*="items in cart" i], [data-testid*="cart-count"], .cart-badge, [class*="cartCount"]');
        const count = confirmation ? await confirmation.textContent() : null;
        // Get product title for confirmation
        const productTitle = await page.evaluate(() => document.querySelector('h1, [class*="product-title"], [class*="productTitle"]')?.textContent?.trim() ?? "");
...
async function handleViewCart() {
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        const cart = await page.evaluate(() => {
            const emptyMsg = document.querySelector('[class*="empty-cart"], [class*="emptyCart"], [data-testid*="empty-cart"], [class*="cart-empty"]');
            ...
            const items = itemEls.map((item) => {
                const title = item.querySelector('[class*="product-name"], [class*="productName"], [class*="item-name"], [class*="itemName"], h3, h4')
                    ?.textContent?.trim() ?? "";
                const price = item.querySelector('[class*="price"], [class*="Price"]')?.textContent?.trim() ?? "";
                const qty = (() => {
                    const qtyInput = item.querySelector('input[aria-label*="quantity" i], input[name*="quantity"]');
                    if (qtyInput)
                        return qtyInput.value;
                    return item.querySelector('[class*="quantity"], [class*="Quantity"], [class*="qty"]')?.textContent?.trim() ?? "1";
                })();
                const brand = item.querySelector('[class*="brand"]')?.textContent?.trim() ?? "";
                return { title, price, qty, brand };
            });
            const subtotal = document.querySelector('[class*="subtotal"], [class*="Subtotal"], [data-testid*="subtotal"]')?.textContent?.trim() ?? "";
            const tax = document.querySelector('[class*="tax"], [class*="Tax"], [data-testid*="tax"]')?.textContent?.trim() ?? "";
            const total = document.querySelector('[class*="total"], [class*="Total"], [class*="order-total"], [class*="orderTotal"]')
                ?.textContent?.trim() ?? "";
        ...
        cart.items.forEach((item, i) => {
            lines.push(`${i + 1}. ${item.title}` + (item.brand ? ` — ${item.brand}` : "") + "\n" +
...
async function handleRemoveFromCart(itemIndex, productName) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        if (itemIndex) {
            ...
            targetTitle = await targetItem.evaluate((el) => el.querySelector('[class*="product-name"], [class*="productName"], [class*="item-name"], h3, h4')?.textContent?.trim() ?? "");
        }
        else if (productName) {
        ...
        const removeBtn = await targetItem.$('button[aria-label*="remove" i], button[class*="remove"], [data-testid*="remove"], button[class*="delete"], [class*="removeItem"]');
        if (!removeBtn) {
        ...
        try {
            const confirmBtn = await page.waitForSelector('button[class*="confirm" i], button[data-testid*="confirm"]', { timeout: 2000 });
            if (confirmBtn) {
                await confirmBtn.click();
...
async function handleSetStore(zipCode, storeId) {
    if (!zipCode && !storeId) {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        if (zipCode && !storeId) {
            try {
                await page.waitForSelector('[class*="store-card"], [class*="storeCard"], [class*="store-result"], [class*="storeResult"], [data-testid*="store"]', { timeout: 15000 });
            }
            catch {
                // Try entering zip in search box if present
                const searchInput = await page.$('input[placeholder*="zip" i], input[placeholder*="city" i], input[id*="store-search"], input[name*="zip"]');
                if (searchInput) {
        ...
        const storeInfo = await page.evaluate((targetStoreId) => {
            ...
            const nameEl = target.querySelector('[class*="store-name"], [class*="storeName"], h3, h4, [class*="heading"]');
            const storeName = nameEl?.textContent?.trim() ?? "Walgreens";
            const addressEl = target.querySelector('[class*="address"], [class*="Address"], address, [class*="store-address"]');
            const address = addressEl?.textContent?.trim() ?? "";
            const idEl = target.querySelector('[data-store-id], [class*="store-id"]');
            const foundStoreId = idEl?.getAttribute("data-store-id") ??
        ...
        (0, session_js_1.saveStore)(store);
        return ok(`Preferred store set:\n` +
            `Store: ${store.storeName}\n` +
            (store.address ? `Address: ${store.address}\n` : "") +
            (store.storeId ? `Store ID: ${store.storeId}\n` : ""));
    });
}
async function handleCheckStoreAvailability(url, productId, zipCode) {
    if (!url && !productId) {
        return err("Provide either url or product_id");
    }
    const store = (0, session_js_1.loadStore)();
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        if (zipCode) {
            try {
                const storeBtn = await page.$('[class*="store-selector"], [aria-label*="store" i], [class*="change-store"], [class*="myStore"], [data-testid*="store-selector"]');
                if (storeBtn) {
                    await storeBtn.click();
                    await page.waitForTimeout(1000);
                    const zipInput = await page.waitForSelector('input[placeholder*="zip" i], input[name*="zip"]', { timeout: 5000 });
        ...
        const availability = await page.evaluate(() => {
            const pickupSection = document.querySelector('[class*="pickup"], [class*="in-store"], [data-testid*="pickup"], [class*="store-availability"], [class*="storeAvailability"], [class*="bopis"]');
            if (!pickupSection) {
                const allAvailability = document.querySelector('[class*="availability"], [class*="Availability"], [class*="fulfillment"], [class*="Fulfillment"]');
                return {
                    inStore: false,
                    online: false,
            ...
            const inStore = !text.includes("not available") &&
                !text.includes("out of stock") &&
                !text.includes("unavailable") &&
                text.length > 0;
            const onlineSection = document.querySelector('[class*="ship"], [class*="delivery"], [data-testid*="shipping"], [class*="online-availability"]');
            ...
            const online = onlineSection
                ...
                : false;
            const storeNameEl = document.querySelector('[class*="store-name"], [class*="storeName"], [class*="my-store"], [class*="myStore"]');
...
async function handleCheckout() {
    ...
    return (0, browser_js_1.withPage)(async (page) => {
        ...
        const cartSummary = await page.evaluate(() => {
            ...
            const items = itemEls.map((item) => {
                const title = item.querySelector('[class*="product-name"], [class*="productName"], [class*="item-name"], h3, h4')
                    ?.textContent?.trim() ?? "";
                const price = item.querySelector('[class*="price"]')?.textContent?.trim() ?? "";
                const qty = (() => {
                    const qtyInput = item.querySelector('input[aria-label*="quantity" i]');
                    if (qtyInput)
                        return qtyInput.value;
            ...
            const tax = document.querySelector('[class*="tax"], [data-testid*="tax"]')?.textContent?.trim() ?? "";
            const total = document.querySelector('[class*="order-total"], [class*="orderTotal"], [class*="total-price"]')
                ?.textContent?.trim() ?? "";
        ...
        const checkoutBtn = await page.$('button[class*="checkout" i], a[class*="checkout" i], [data-testid*="checkout"], a[href*="/checkout"]');
        let checkoutUrl = "https://www.walgreens.com/checkout/";
        if (checkoutBtn) {
            const href = await checkoutBtn.getAttribute("href");
            if (href)
                checkoutUrl = href.startsWith("http") ? href : `https://www.walgreens.com${href}`;
        }
        ...
        const checkoutSummary = await page.evaluate(() => {
            const subtotal = document.querySelector('[class*="subtotal"], [data-testid*="subtotal"]')?.textContent?.trim() ?? "";
            const tax = document.querySelector('[class*="tax"], [data-testid*="tax"]')?.textContent?.trim() ?? "";
            const shipping = document.querySelector('[class*="shipping"], [data-testid*="shipping-cost"]')?.textContent?.trim() ?? "";
            const total = document.querySelector('[class*="order-total"], [class*="orderTotal"], [class*="grand-total"], [data-testid*="total"]')
                ?.textContent?.trim() ?? "";
            const deliveryInfo = document.querySelector('[class*="delivery-address"], [class*="deliveryAddress"], [class*="shipping-address"]')
        ...
        if (totals)
            summary.push(`Subtotal: ${totals}`);
        if (checkoutSummary.shipping)
            summary.push(`Shipping: ${checkoutSummary.shipping}`);
        if (checkoutSummary.tax || cartSummary.tax)
            summary.push(`Tax: ${checkoutSummary.tax || cartSummary.tax}`);
        if (checkoutSummary.total || cartSummary.total)
            summary.push(`Total: ${checkoutSummary.total || cartSummary.total}`);
        if (checkoutSummary.deliveryInfo)
            summary.push(`\nDelivery to: ${checkoutSummary.deliveryInfo}`);
        summary.push("\n⚠️  This is a checkout summary — the order has NOT been placed.", "To complete your purchase, visit: " + page.url(), "\nNote: This MCP tool returns summaries only and does not auto-confirm orders.");
