...
const SESSION_DIR = path_1.default.join(os_1.default.homedir(), ".striderlabs", "walgreens");
const COOKIES_FILE = path_1.default.join(SESSION_DIR, "cookies.json");
const AUTH_FILE = path_1.default.join(SESSION_DIR, "auth.json");
const STORE_FILE = path_1.default.join(SESSION_DIR, "store.json");
...
function saveStore(info) {
    ensureSessionDir();
    fs_1.default.writeFileSync(STORE_FILE, JSON.stringify(info, null, 2));
}
function loadStore() {
    if (!fs_1.default.existsSync(STORE_FILE))
        return null;
    try {
        return JSON.parse(fs_1.default.readFileSync(STORE_FILE, "utf-8"));
    }
    catch {
        return null;
    }
